...
DebugRenderer::DebugRenderer(ID3D11Device* device)
{
	...
	//AABBメッシュ作成
	AABBMesh(device, 1.0f, 1.0f, 0.0f, 1.0f, 16, 1);
}
...
void DebugRenderer::Render(ID3D11DeviceContext* context, const DirectX::XMFLOAT4X4& view, const DirectX::XMFLOAT4X4& projection)
{
	...
	//AABB描画
	context->IASetVertexBuffers(0, 1, aabbVertexBuffer.GetAddressOf(), &stride, &offset);
	for (const Boundy& boundy : aabbs)
	{
		// ワールドビュープロジェクション行列作成
		DirectX::XMMATRIX S = DirectX::XMMatrixScaling(boundy.radius, boundy.height, boundy.radius);
		DirectX::XMMATRIX T = DirectX::XMMatrixTranslation(boundy.position.x, boundy.position.y, boundy.position.z);
		DirectX::XMMATRIX W = S * T;
		DirectX::XMMATRIX WVP = W * VP;

		// 定数バッファ更新
		CbMesh cbMesh;
		cbMesh.color = boundy.color;
		DirectX::XMStoreFloat4x4(&cbMesh.wvp, WVP);

		context->UpdateSubresource(constantBuffer.Get(), 0, 0, &cbMesh, 0, 0);
		context->Draw(aabbVertexCount, 0);
	}
	aabbs.clear();
}
...
//AABB描画
void DebugRenderer::AABB(const DirectX::XMFLOAT3& position, float radius, float height, const DirectX::XMFLOAT4& color)
{
	Boundy aabb;
...
//AABBメッシュ描画
void DebugRenderer::AABBMesh(ID3D11Device* device, float radius1, float radius2, float start, float height, int slices, int stacks)
{
	aabbVertexCount = 2 * slices * (stacks + 1) + 2 * slices;
	std::unique_ptr<DirectX::XMFLOAT3[]> vertices = std::make_unique<DirectX::XMFLOAT3[]>(aabbVertexCount);
	...
	// 頂点バッファ
	{
		...
		desc.ByteWidth = static_cast<UINT>(sizeof(DirectX::XMFLOAT3) * aabbVertexCount);
		desc.Usage = D3D11_USAGE_IMMUTABLE;	// D3D11_USAGE_DEFAULT;
		desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		desc.CPUAccessFlags = 0;
		desc.MiscFlags = 0;
		...
		HRESULT hr = device->CreateBuffer(&desc, &subresourceData, aabbVertexBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
}